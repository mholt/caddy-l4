---
title: Postgres TLS Handler
---

# Postgres TLS Handler

## Summary

The `postgres_tls` handler performs the PostgreSQL `SSLRequest` negotiation
so that a following [`tls`](tls.md) handler can terminate TLS on a *classic*
PostgreSQL connection. Once TLS is terminated, the handlers and matchers that
follow operate on the **cleartext** startup message, so traffic can be routed by
user, database, or application name.

PostgreSQL does not begin TLS with a TLS ClientHello. A libpq client first sends
an 8-byte `SSLRequest` and waits for a single byte — `S` (the server will use
TLS) or `N` (continue in plaintext) — and only then starts the TLS handshake.
Because the generic [`tls`](tls.md) handler expects a ClientHello immediately, it
cannot terminate such a connection on its own. This handler reads the
`SSLRequest`, replies `S`, and hands the connection to the next handler (no TLS
code is duplicated — it relies on the `tls` handler for the actual termination).

It must be preceded by a matcher that confirms the connection is a Postgres
`SSLRequest` (e.g. the `postgres` matcher), and followed by a `tls` handler.

> Note: PostgreSQL 17+ can negotiate TLS directly (advertising the ALPN value
> `postgresql`), in which case the standard `tls` handler/matcher is sufficient
> and this handler is not needed — see the
> [Postgres-over-TLS example](../examples/postgres-over-tls.md). Use
> `postgres_tls` for the classic `SSLRequest` flow used by earlier servers
> and by clients that don't request direct TLS.

## Syntax

The handler takes no arguments and no block:

```caddyfile
postgres_tls
```

### Caddyfile

```caddyfile
{
    layer4 {
        :5432 {
            # match Postgres connections, negotiate the SSLRequest,
            # terminate TLS, then proxy the (now cleartext) connection
            @pg postgres
            route @pg {
                postgres_tls
                tls
                proxy upstream.local:5432
            }
        }
    }
}
```

The config above is incomplete unless a server TLS certificate is obtained or
generated by Caddy.

### JSON

JSON equivalent to the Caddyfile config provided above:

```json
{
    "apps": {
        "layer4": {
            "servers": {
                "srv0": {
                    "listen": [
                        ":5432"
                    ],
                    "routes": [
                        {
                            "match": [
                                {
                                    "postgres": {}
                                }
                            ],
                            "handle": [
                                {
                                    "handler": "postgres_tls"
                                },
                                {
                                    "handler": "tls"
                                },
                                {
                                    "handler": "proxy",
                                    "upstreams": [
                                        {
                                            "dial": [
                                                "upstream.local:5432"
                                            ]
                                        }
                                    ]
                                }
                            ]
                        }
                    ]
                }
            }
        }
    }
}
```

## Re-originating TLS to the upstream

The handler above terminates TLS for inbound matching. If the upstream itself
expects TLS (`sslmode != disable`), the connection must be re-encrypted with the
same `SSLRequest` → `S` negotiation *outbound*. That counterpart is available
programmatically as `TLSClient(conn, *tls.Config)` in this package; wiring
it into the proxy behind an opt-in upstream option is left as a follow-up.
Deployments where the hop to the backend is already on a trusted/encrypted
network (e.g. an overlay such as WireGuard) only need the inbound termination.
